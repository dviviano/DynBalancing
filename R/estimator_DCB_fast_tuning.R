...
## n_beta_sparse: how many coefficients to prioritize for balancing if no sparsity is imposed over the estimation?
...
## It is a version of compute_estimator with faster (but less precise) computations for the choice
## of the tuning parameters, consisting of setting the second constant for the smaller coefficients
## equal to 10 times the first constant (instead of choosing also the second constant)

compute_estimator_fast_adaptive <- function(Covariates_t, Y_T, Ds, ds, Time, params = NA, tolerance_constraint1 = 10**(-8),
  ...
  K <- seq(from = lb, to = ub, length = length_grid)
  ...
  for(k1 in K){
    if(open_source){
      gammas_first <- tryCatch(compute_gamma1_os(X1 = Covariates_t_nonna[[1]],
                                                 ...
                                                 with_beta = with_beta, beta_hat = beta_hat, K_2 = 10 * k1,
    ...
    suppressWarnings(if(any(is.na(gammas_first) == F)){break})
  }

  suppressWarnings(if(any(is.na(gammas_first))){  stop('Error: infeasible problem for period 1. Try to increase the ub parameter.') })
      ...
      for(k1 in K){
        if(open_source){
          gammas_tj <- tryCatch(compute_gammat_os(gammat_before = keep_gammas[not_nas[[t]],t-1],
                                                  ...
                                                  with_beta = with_beta, beta_hat = beta_hat,
                                                  K_2 = 10 * k1, n_beta_nonsparse = n_beta_nonsparse,
                                                  ratio_coefficients = ratio_coefficients), error = function(e){NA})
        } else{
        ...
        suppressWarnings(if(any(is.na(gammas_tj) == F)){break})
      }

      suppressWarnings(if(any(is.na(gammas_tj))){  stop(paste0('Error: infeasible problem for period ', t,  '. Try to increase the ub parameter.')) })
